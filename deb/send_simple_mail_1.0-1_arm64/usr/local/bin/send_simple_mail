...
""" 
creating parser for arguments (-es --emailsender name of section in config file with creditials, 
-a --address for email of recipient, -s --subject for subject of mail, -t --text for email text, 
-f --files for attachments). 
-a and -s are mandatory, if section DEFAULT is defined in config then youcan skip -es,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v", "--version", action="version", version="send_simple_mail 1.0"
    )
    parser.add_argument(
        "-es", "--emailsender", help="name of email sender configured in config"
    )
    parser.add_argument(
        "-a", "--address", required=True, nargs="+", help="recipient(s) address(es)"
    )
    ...
    parser.add_argument(
        "-fb", "--filesbinary", nargs="+", help="adding binary file attachment "
    )
    ...
    try:
        mail = Mail(
            mailconf["EMAIL"], mailconf["PASS"], mailconf["SMTP"], mailconf["PORT"]
        )
    except KeyError:
        sys.exit(
